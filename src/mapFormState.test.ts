...
import { DEFAULT_CONFIG, DEFAULT_REMORTGAGE_GAP_MONTHS, DEFAULT_SAVINGS_PAYOUT_INTERVAL_YEARS } from './api/defaults';
...
describe('mapInputsToFormState / mapFormStateToInputs round trip', () => {
  it('round-trips a fully-populated MortgageInputs exactly', () => {
    const form = mapInputsToFormState(FULL_INPUTS, DEFAULT_FORM_STATE);
  ...
  it('falls back to engine defaults, not 0, for fields that are invalid at 0', () => {
    ...
    const form = mapInputsToFormState(minimal, DEFAULT_FORM_STATE);

    // 0 would be rejected by validate_inputs (savingsPayoutIntervalYears must
    // be > 0) — the real regression this test targets.
    expect(form.savingsPayoutIntervalYears).toBe(String(DEFAULT_SAVINGS_PAYOUT_INTERVAL_YEARS));
    expect(form.remortgageGapMonths).toBe(String(DEFAULT_REMORTGAGE_GAP_MONTHS));
  ...
  it('does not let an explicit-null config field (as the API actually returns for unset fields) clobber DEFAULT_CONFIG', () => {
    ...
    const form = mapInputsToFormState(inputs, DEFAULT_FORM_STATE);

    expect(form.arrangementFee).toBe('999');
    expect(form.annualOverpaymentAllowancePct).toBe(String(DEFAULT_CONFIG.annualOverpaymentAllowancePct));
    expect(form.allowanceBasis).toBe(DEFAULT_CONFIG.allowanceBasis);
    expect(form.ercRateOnExcessPct).toBe(String(DEFAULT_CONFIG.ercRateOnExcessPct));
    expect(form.ercAppliesDuringFixedTermOnly).toBe(DEFAULT_CONFIG.ercAppliesDuringFixedTermOnly);
    expect(form.arrangementFeeAddedToLoan).toBe(DEFAULT_CONFIG.arrangementFeeAddedToLoan);
  });
  ...
  it('preserves UI-only fields (isFirstTimeBuyer, showAdvanced) from the current form', () => {
    ...
    const form = mapInputsToFormState(FULL_INPUTS, currentForm);
