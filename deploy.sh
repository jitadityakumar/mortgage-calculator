...
echo "==> Building backend image: $BACKEND_IMAGE_NAME:latest"
docker build -t "$BACKEND_IMAGE_NAME:latest" "$SCRIPT_DIR/backend"

echo "==> Building frontend image: $FRONTEND_IMAGE_NAME:latest"
docker build -t "$FRONTEND_IMAGE_NAME:latest" -f "$SCRIPT_DIR/frontend/Dockerfile" "$SCRIPT_DIR"

echo "==> Transferring images to $REMOTE_HOST (this may take a minute)"
docker save "$BACKEND_IMAGE_NAME:latest" "$FRONTEND_IMAGE_NAME:latest" | gzip | ssh "$REMOTE" docker load
